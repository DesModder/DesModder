...
export default class Controller {
  ...
  drawGlesmosSketchToCtx(
    ...
    for (let i = 0; i < compiledGL.defs.length; i++) {
      // outlines will give us problems if we group these
      try {
        if (this.canvas?.element) {
          this.canvas.updateTransforms(transforms);
          this.canvas?.buildGLesmosShaders(id, {
            deps,
            def: compiledGL.defs[i],
            color: compiledGL.colors[i],
            line_color: compiledGL.line_colors[i],
            line_width: compiledGL.line_widths[i],
          });
          this.canvas?.render();
