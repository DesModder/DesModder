...
function compileShader(gl: WebGLRenderingContext | WebGL2RenderingContext, shaderCode: string, type: number) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        showGLesmosError(`Error compiling ${type === gl.VERTEX_SHADER ? "vertex" : "fragment"} shader:`);
...
function buildShaderProgram(gl: WebGLRenderingContext | WebGL2RenderingContext, vert: string, frag: string) {
...
function setUniform(gl: WebGLRenderingContext | WebGL2RenderingContext, program: WebGLProgram, uniformName: string, uniformType: UniformType, uniformValue: number | number[]) {
    let uniformSetterKey: keyof WebGLRenderingContext = ("uniform" + uniformType) as keyof WebGLRenderingContext;
    (gl[uniformSetterKey] as Function)(gl.getUniformLocation(program, uniformName), uniformValue);
}
...
export function initGLesmosCanvas(): GLesmosCanvas {
    ...
    let gl: WebGLRenderingContext = c.getContext("webgl") as WebGLRenderingContext;
    ...
    let testShaderProgram = buildShaderProgram(gl, VERTEX_SHADER, TEST_FRAGMENT_SHADER);
