...
export default function exportAsGLesmos() {
  ...
  return glSnippets.filter(s => s[1] == "function").map(s=>s[0]).join("\n\n") + 
  `\n\nvec4 outColor = vec4(1.0);\n\n`+
  `void glesmosMain(vec2 coords) { float x = coords.x; float y = coords.y;\n\n    ` +
  glSnippets.filter(s => s[1] == "expression").map(s=>s[0]).join("\n    ") + 
  "\n\n}\n\n" +
  glSnippets.filter(s => s[1] == "unimplemented").map(s=>s[0]).join("\n\n");
}
    ...
    case "expression":
      ...
      switch (parsed.type) {
        case "FunctionDefinition":
          return [(
            `float ${parsed._symbol}(${parsed._argSymbols.map(s => "float "+s).join(", ")}) {\n` +
            `  return ${childExprToGL(parsed._expression)};\n` +
            `}`
          ), "function"];
        case "Assignment":
          return [`float ${parsed._symbol} = ${childExprToGL(parsed._expression)};`, "expression"];
        case "Comparator['<']":
          ...
          return [`if (${childExprToGL(parsed.args[0] as ChildExprNode)} - ${childExprToGL(parsed.args[1] as ChildExprNode)} < 0.0) {\n`+
          `    outColor.rgb = mix(outColor.rgb, vec3(${r}, ${g}, ${b}), ${a});\n`+
          `}`, "expression"]
        default:
          return [`// Unimplemented root level type: ${parsed.type}`, "unimplemented"];
      }
    default:
      return [`// Unimplemented item model type: ${expr.type}`, "unimplemented"];
  }
}
