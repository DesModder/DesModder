export function wolfram2desmos(input) {
  ...
  while (find(/\//g) != -1) {
    ...
    // implement the denominator; after the slash
    {
      ...
      let isFunction =
        startingIndex ==
        find(
          /\/((\-)|([A-Z|Α-ω|ϕ|א-ת|Ⓐ-Ⓩ|√|∞|\_])|(\-([A-Z|Α-ω|ϕ|א-ת|Ⓐ-Ⓩ|√|∞|\_])))(\(|\{)/gi
        );
      if (isFunction) {
        insert(i, "{(");
        i += 3;
  ...
  while (
    find(
      /(sum|prod(uct|))_\([A-Z|a-z|\d|Α-ω|∞|א-ת|Ⓐ-Ⓩ|\_|\\]+\s*=\s*[A-Z|a-z|\d|Α-ω|∞|א-ת|Ⓐ-Ⓩ|\_|\\]+\)/g
    ) != -1
  ) {
    i =
      find(
    ...
    if (isOneArgument) {
      ...
      // after the modulus
      {
        ...
        let isFunction =
          startingIndex ==
          find(
            /mod((\-)|([A-Z|Α-ω|ϕ|א-ת|√|∞|\_])|(\-([A-Z|Α-ω|ϕ|א-ת|√|∞|\_])))(\(|\{)/gi
          );
        if (isFunction) {
          insert(i, "(");
          i += 2;
  ...
  while (find(/[א-ת|Ⓐ-Ⓩ]\^\{/g) != -1) {
    ...
    while (i < input.length) {
      ...
      if (bracket == 0) {
        ...
        if (
          input[i + 1] != "(" ||
          (input[startingIndex].match(/[ס-ר|Ⓖ-Ⓛ]/g) != null &&
            (slicedPortion == "{-1}" || slicedPortion == "{2}"))
        ) {
        ...
        // if there is no "(" after the exponent || it's arctrig^{2}, then drop this.
          ...
          input[i + 1] != "(" ||
          (input[startingIndex].match(/[ט-נ|Ⓐ-Ⓕ]/g) != null &&
            slicedPortion == "{2}")
        ) {
          overwrite(startingIndex + 1, "↑");
          i = input.length;
