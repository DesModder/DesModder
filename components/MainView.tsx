import { DCGView, If, Tooltip, Calc, jquery, keys } from "desmodder";
...
export default class MainView extends DCGView.Class<{
  ...
  template() {
    return (
      <div class="video-creator-main-view-container">
        <Tooltip tooltip="Video Creator menu" gravity="w">
          <div
  ...
  didMountContainer() {
    ...
    jquery(document).on(
      "dcg-tapstart.video-creator-main-view wheel.video-creator-main-view",
      (e: Event) => {
    ...
    jquery(document).on(
      "keydown.video-creator-main-view",
      (e: KeyboardEvent) => {
        if (
          keys.lookup(e) === "Esc" &&
